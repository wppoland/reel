...
final class ReelService implements HasHooks
{
     ...
     * own. The shortcode/block can appear on any page, so cache the built
     * markup in a short-lived transient keyed by product, video URL and
     * autoplay so an uncached page view never blocks on a remote oEmbed fetch.
     * Self-hosted files build locally and are cheap, but caching them too keeps
     * the path uniform. Keying on a hash of the URL + autoplay auto-invalidates
     * when either changes; empty results are cached briefly so a transient
     * oEmbed failure is retried soon rather than stuck blank.
     */
    public function renderVideoHtml(\WC_Product $product): string
    {
        ...
        $autoplay = (bool) ($this->videoSettings()['autoplay'] ?? false);

        // Key on a hash of the URL + autoplay so changing the meta or toggling
        // autoplay invalidates the cache automatically. Bounded transient name.
